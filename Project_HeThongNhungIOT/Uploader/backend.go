...
func uploadHandler(c *gin.Context) {
	...
	if len(metadataStr) > 0 {
		...
		currentTS := uint64(time.Now().UTC().UnixMilli())
...
func statsHandler(c *gin.Context) {
	...
	err := db.QueryRow("SELECT COUNT(*), COALESCE(SUM(visit_count), 0) FROM face_data").Scan(&totalPeople, &totalVisits)
	if err != nil {
		logger.Error("Stats query error", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	todayStart := time.Now().UTC().Add(-24 * time.Hour).UnixMilli()
	var newToday, returningToday int
	db.QueryRow("SELECT COUNT(*) FROM face_data WHERE first_seen_ts >= ?", todayStart).Scan(&newToday)
	db.QueryRow("SELECT COUNT(*) FROM face_data WHERE last_seen_ts >= ? AND first_seen_ts < ?", todayStart, todayStart).Scan(&returningToday)
